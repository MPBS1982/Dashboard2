...
import ExportButton from '../SalesAnalytics/ExportButton'

type Metric = 'qtd' | 'faturamento' | 'vr_liquido' | 'preco_medio'

export default function ProductByUnitComparison({ data, selectedProduct }: { data: SalesRecord[]; selectedProduct: string }) {
  const units = useMemo(() => Array.from(new Set(data.map(d => d.unidade))).filter(Boolean), [data])
  const [months, setMonths] = useState<number>(6)
  const [metric, setMetric] = useState<Metric>('qtd')
  ...
  const period = `${format(subMonths(latestDate, months), 'yyyy-MM-dd')} a ${format(latestDate, 'yyyy-MM-dd')}`
  const exportRows = chartData.map(r => ({ unidade: r.unidade, produto: selectedProduct, metric, valor: (r as any)[metric], periodo: period }))

  return (
    <div className="bg-white rounded-lg shadow p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-500">Comparativo de Produtos por Unidade</div>
      <div className="flex items-center gap-2">
        ...
        <select className="border rounded px-2 py-1" value={months} onChange={e => setMonths(Number(e.target.value))}>
          ...
          <ExportButton rows={exportRows} filenameBase={`produtos_${unidade}_${months}m`} columns={["unidade","produto","metric","valor","periodo"]} />
        </div>
      </div>
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey={metric} fill="#f59e0b" />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
